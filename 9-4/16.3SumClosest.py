#Runtime: 230 ms, faster than 99.32% of Python online submissions for 3Sum Closest.
#Memory Usage: 13.5 MB, less than 79.59% of Python online submissions for 3Sum Closest.
...
class Solution(object):
    def threeSumClosest(nums, target):
        ...
        elif nums[-2]+nums[-1]+nums[-3]<target:
        ...
        while True:
            ...
            if c in b:
                ...
                while True:
                    ...
                    if len(b)<=2:
                        try: 
                            if min(abs(c-b[bf]),abs(c-b[ed]),abs(result-target))==abs(c-b[bf]):
                                result=nums[i]+nums[j]+b[bf]
                            elif min(abs(c-b[bf]),abs(c-b[ed]),abs(result-target))==abs(c-b[ed]):
                                result=nums[i]+nums[j]+b[ed]
                        except:
                            if min(abs(c-b[bf]),abs(c-b[ed]))==abs(c-b[bf]):
                                result=nums[i]+nums[j]+b[bf]
                            else:
                                result=nums[i]+nums[j]+b[ed]
                        break
                    ...
                    if b[bf] > c:
                        ...
                        bf=ed
                        b=[b[ed]]
                    elif b[bf] <= c and c <b[int(len(b)/2)]:
                        ed=int(len(b)/2)
                        b=b[0:int(len(b)/2)]
                    else:
                        bf=int(len(b)/2)
            ...
            j0=j
            j=j+1
            if j>len(nums)-1 or nums[j] > target-nums[i]-nums[j0] :
                i0=i
                while True:
                    i=i+1
                    if i>=len(nums)-3 or nums[i] != nums[i0]:
